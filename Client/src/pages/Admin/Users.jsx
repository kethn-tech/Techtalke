...
const Users = () => {
  ...
  const getRoleColor = (role) => {
    switch (role) {
      case 'admin': return 'from-orange-500/20 to-red-500/20 text-orange-300 border-orange-500/30';
      case 'moderator': return 'from-purple-500/20 to-indigo-500/20 text-purple-300 border-purple-500/30';
      default: return 'from-cyan-500/20 to-blue-500/20 text-cyan-300 border-cyan-500/30';
    }
  };
            ...
            <div className="bg-slate-800/40 backdrop-blur-2xl rounded-3xl border border-slate-700/50 shadow-2xl shadow-black/20 overflow-hidden">
              <div className="p-8">
                <div className="flex items-center gap-6">
                  ...
                  <Button
                    variant="outline"
                    className="h-14 px-6 border-slate-600/50 bg-slate-800/50 text-slate-300 hover:bg-slate-700/70 hover:border-indigo-500/50 hover:text-indigo-300 rounded-2xl transition-all duration-300 backdrop-blur-sm"
                  >
                      ...
                      >
                              ...
                              <div className="flex items-center gap-2">
                                <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${
                                  ...
                                  <div className={`w-2 h-2 rounded-full ${
                                    user.profileSetup ? "bg-emerald-400" : "bg-amber-400"
                                  }`}></div>
                        ...
                        <td className="px-8 py-6">
                          <div className="space-y-1">
                            ...
                            <div className="text-slate-400 text-sm">Primary contact</div>
                          </div>
                        </td>
                        
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-3">
                            <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold border ${getRoleColor(user.role)} shadow-lg`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-slate-300 px-4 py-2 bg-gradient-to-r from-slate-700/70 to-slate-600/70 rounded-xl border border-slate-600/30 font-medium">
                      <span className="text-cyan-400 font-bold">{pagination.page}</span>
        ...
        <DialogContent className="bg-slate-800/95 backdrop-blur-2xl border-slate-700/50 text-white max-w-lg shadow-2xl rounded-3xl">
          <DialogHeader className="space-y-4 pb-6">
            ...
            <DialogDescription className="text-slate-300 text-lg">
              Create a new user account with specified permissions and access levels
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <label className="text-sm font-semibold text-slate-300">User Role & Permissions</label>
              <div className="grid grid-cols-3 gap-3">
                {[
                  { role: "user", icon: <User className="h-4 w-4" />, desc: "Standard access" },
                  { role: "admin", icon: <Crown className="h-4 w-4" />, desc: "Full control" },
                  { role: "moderator", icon: <Shield className="h-4 w-4" />, desc: "Limited admin" }
                ].map((r) => (
                  <Button
                    key={r.role}
                    variant={newUserData.role === r.role ? "default" : "outline"}
                    className={
                    ...
                    onClick={() => setNewUserData({ ...newUserData, role: r.role })}
                  >
                    <div className="flex flex-col items-center gap-2">
                      {r.icon}
                      <div className="text-center">
                        <div className="font-semibold">{r.role.charAt(0).toUpperCase() + r.role.slice(1)}</div>
            ...
            {selectedUser && (
              <div className="flex items-center gap-4 p-6 rounded-2xl bg-gradient-to-r from-red-500/10 to-pink-500/10 border border-red-500/20">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-slate-400 font-medium">{selectedUser.email}</div>
        ...
        <DialogContent className="bg-slate-800/95 backdrop-blur-2xl border-slate-700/50 text-white shadow-2xl rounded-3xl max-w-md">
          ...
          <div className="py-6 space-y-6">
            ...
            <div className="space-y-3">
              <label className="text-sm font-semibold text-slate-300">Select New Role</label>
              <div className="grid grid-cols-2 gap-4">
                {[
                  { role: "user", icon: <UserCheck className="h-5 w-5" />, color: "from-cyan-500 to-blue-500", desc: "Standard user access" },
                  { role: "admin", icon: <Crown className="h-5 w-5" />, color: "from-orange-500 to-red-500", desc: "Full administrative control" }
                ].map((r) => (
                  <Button
                    key={r.role}
                  ...
                  >
                    <div className="flex flex-col items-center gap-3">
                      {r.icon}
                      <div className="text-center">
                        <div className="font-bold">{r.role.charAt(0).toUpperCase() + r.role.slice(1)}</div>
                        <div className="text-xs opacity-70">{r.desc}</div>
                      </div>
                    </div>
                  </Button>
