  ...
  try {
    ...
    if (!cachedSettings || Date.now() - cacheTimestamp > CACHE_TTL) {
      try {
        ...
        console.log(`🔄 Maintenance settings refreshed: ${cachedSettings?.maintenanceMode ? 'ON' : 'OFF'}`);
      } catch (dbError) {
    ...
    console.log(`🚧 Maintenance mode is ON - checking request: ${req.method} ${req.originalUrl}`);
    ...
    if (token) {
      try {
        ...
        if (user && user.role === "admin") {
          isAdmin = true;
          console.log(`✅ Admin user ${user.email} accessing during maintenance`);
        } else if (user) {
          console.log(`🚫 Non-admin user ${user.email} blocked during maintenance`);
        }
      } catch (jwtError) {
        console.log('❌ Invalid token during maintenance check:', jwtError.message);
    ...
    console.log(`🚫 Blocking non-admin request: ${req.method} ${req.originalUrl}`);
