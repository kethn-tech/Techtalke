...
const setupSocket = (io) => {
  ...
  const getGroupList = async (userId) => {
    try {
      ...
      const groupsWithLastMessage = await Promise.all(userGroups.map(async (group) => {
  ...
  const sendMessage = async (message) => {
    try {
      ...
      if (recipientSocketId) {
        ...
        console.log(`Sent message ${messageData._id} to recipient ${message.recipient}`);
      }
    } catch (error) {
  ...
  const markMessagesAsRead = async (data) => {
    try {
      ...
      console.log(`Attempting to mark messages as read - reader: ${reader}, chatId: ${chatId}, chatType: ${chatType}`);
        ...
        if (chatType === 'group') {
          ...
          if (group) {
            group.members.forEach(memberId => {
              const memberSocketId = userSocketMap.get(memberId.toString());
              if (memberSocketId) {
                io.to(memberSocketId).emit('messagesRead', { messageIds, reader, chatType, chatId });
              }
            });
          }
        } else {
          ...
          if (senderSocketId) {
            console.log(`Emitting messagesRead to sender ${chatId} with ${messageIds.length} messages`);
            io.to(senderSocketId).emit('messagesRead', { messageIds, reader, chatType, chatId });
          }

          if (readerSocketId) {
            console.log(`Emitting messagesRead to reader ${reader} with ${messageIds.length} messages`);
            io.to(readerSocketId).emit('messagesRead', { messageIds, reader, chatType, chatId });
          }
        }
      } else {
  ...
  const handleTypingStatus = (data) => {
    ...
    const typingKey = chatType === 'group' ? `${sender}-${chatId}-group` : `${sender}-${chatId}`;
    if (isTyping) {
    ...
    if (chatType === 'group') {
      ...
      const Group = require('./models/GroupModel');
      Group.findById(chatId).then(group => {
        if (group) {
          group.members.forEach(memberId => {
        ...
        if (updatedMessage.group) {
          ...
          if (group) {
            group.members.forEach(memberId => {
              const memberSocketId = userSocketMap.get(memberId.toString());
              if (memberSocketId) {
                io.to(memberSocketId).emit("reactionUpdated", { message: updatedMessage });
              }
            });
          }
        } else {
          ...
          const recipientSocketId = userSocketMap.get(updatedMessage.recipient._id.toString());

          if (senderSocketId) {
            io.to(senderSocketId).emit("reactionUpdated", { message: updatedMessage });
          }
          if (recipientSocketId) {
            io.to(recipientSocketId).emit("reactionUpdated", { message: updatedMessage });
          }
        }
      } catch (error) {
